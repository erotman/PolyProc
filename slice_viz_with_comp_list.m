function [] = slice_viz_with_comp_list(gid_map, numElement, varargin)
...
elseif any(strcmp(varargin,'completeness')) 
    %coloring based on average completeness
    idx = find(strcmp(varargin,'completeness'))+1;
    full_comp_mean = [0,0;0,0;numElement(:,1), varargin{idx}];
    comp_mean = zeros(length(gid_list),1);
    for i = 1:length(gid_list)
        comp_mean(i) = full_comp_mean(full_comp_mean(:,1)==gid_list(i), 2);
    end
    comp_color = jet(101);
    for i = 1:length(gid_list)
        color_gid_list(i,:) = ...
...
for k = 1:length(z_pos)
            ...
            axis off;
            % colorbar setup
                if any(strcmp(varargin,'completeness'))
                    if(size(comp_mean,1) > 1)
                        colormap(comp_color)
